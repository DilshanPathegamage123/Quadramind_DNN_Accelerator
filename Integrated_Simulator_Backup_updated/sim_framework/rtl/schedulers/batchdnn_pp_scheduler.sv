    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (!mt_active && mt_cq_cnt > 0) begin
                ...
                automatic logic [LAYER_ID_WIDTH-1:0] dist_val;
                ...
                for (int k = 0; k < mt_cq_cnt && k < QD; k++) begin
                    ...
                    if (!sched_table[try_cand].mem_access_done_flag) begin
                        ...
                        dist_val = layer_distance(try_cand, ct_current_layer[try_dnn]);
                        ...
                        if (dist_val <= MAX_LAYER_DISTANCE &&
                        ...
                        end else if (dist_val > MAX_LAYER_DISTANCE) begin
